...
if sys.version_info < (3, 2):
    print(
        "This antlr3 module requires Python 3.2 or later. You can "
                ...
                for root, dirs, files in os.walk(path, topdown=True):
                    ...
                    for name in graveyard:
                        ...
                        try:
                            ...
                            log.warn("Failed to delete '%s': %s", filePath, exc)
...
class unittest(Command):
    ...
    def run(self):
        ...
        for testPath in testFiles:
            ...
            try:
                modFile, modPathname, modDescription = imp.find_module(
                    testID, [testDir]
                )

                testMod = imp.load_module(testID, modFile, modPathname, modDescription)

                suite.addTests(unittest.defaultTestLoader.loadTestsFromModule(testMod))

            except Exception:
                buf = io.StringIO()
                traceback.print_exc(file=buf)

                loadFailures.append((os.path.basename(testPath), buf.getvalue()))
        ...
        for testName, error in loadFailures:
            sys.stderr.write("\n" + "=" * 70 + "\n")
            sys.stderr.write(f"Failed to load test module {testName}\n")
...
class functest(Command):
    ...
    user_options = [
        ("testcase=", None, "testcase to run [default: run all]"),
        (
            "antlr-version=",
            None,
    ...
    def run(self):
        ...
        if self.antlr_jar is not None:
            ...
            classpath = [
                os.path.join(rootDir, "archive", f"antlr-{self.antlr_version}.jar")
            ]
        ...
        for testPath in glob.glob(os.path.join(testDir, test_glob)):
            ...
            # tests
            if self.testcase is not None and not os.path.basename(testPath)[
                :-3
            ].startswith(self.testcase):
                continue
...
setup(
    ...
    cmdclass={"unittest": unittest, "functest": functest, "clean": clean},
)
