...
from antlr3.tree import INVALID_TOKEN_TYPE, CommonTree, CommonTreeAdaptor
from antlr3.treewizard import (
    ARG,
    BEGIN,
    COLON,
    DOT,
...
class TestTreeWizard(unittest.TestCase):
    ...
    def testInit(self):
        ...
        self.assertEqual(wiz.tokenNameToTypeMap, {"a": 0, "b": 1})
    ...
    def testGetTokenType(self):
        ...
        self.assertEqual(wiz.getTokenType("A"), 5)

        self.assertEqual(wiz.getTokenType("VAR"), 11)

        self.assertEqual(wiz.getTokenType("invalid"), INVALID_TOKEN_TYPE)
    ...
    def __simplifyIndexMap(self, indexMap):
        return {  # stringify nodes for easy comparing
            ttype: [str(node) for node in nodes] for ttype, nodes in indexMap.items()
        }
    ...
    def testRepeatsVisitWithContext(self):
        ...
        def visitor(node, parent, childIndex, labels):
            elements.append(f"{node}@{parent}[{childIndex}]")
    ...
    def testRepeatsVisitWithNullParentAndContext(self):
        ...
        def visitor(node, parent, childIndex, labels):
            elements.append("{}@{}[{}]".format(node, parent or "nil", childIndex))
